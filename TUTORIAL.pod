...
So, create I</srv/sandboxes/nb10/etc/mk.conf> file.
    ...
    PACKAGES=               /srv/pkgsrc_bin/NetBSD/${PKGSRC_ARCH}/10-current
    PKGSRCDIR=              /srv/pkgsrc/current
...
Of course, you can modify I<mk.conf> if needed.
...
For this to work we need libkver package inside chroot.
...
Now we are ready for running bulk build with one sandbox. Let's start with building one package.

    you$ $HOME/pkg_distbb/bin/bmake -p lang/gawk -x upload_pkgs

See B<distbb> options for details.
